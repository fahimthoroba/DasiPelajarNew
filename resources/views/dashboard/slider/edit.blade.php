...
@section('content')
    <div class="max-w-2xl">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-display font-bold text-gray-900 dark:text-white">Edit Slide</h1>
        </div>

        <form action="{{ route('dashboard.slider.update', $slider) }}" method="POST" enctype="multipart/form-data"
            ...
            <div x-data="imagePreview()">
                ...
                <div
                    ...
                    <div class="absolute inset-0 z-10 flex flex-col items-center justify-center text-gray-400 group-hover:text-emerald-500 transition-colors"
                        ...
                        <span class="text-xs font-medium">Ganti Gambar (Opsional)</span>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="label" value="{{ old('label', $slider->label ?? 'Suara Pelajar Kediri') }}"
                ...
                <div>
                    ...
                    <input type="text" name="judul_utama" value="{{ old('judul_utama', $slider->judul_utama) }}" required
            ...
            <div>
                ...
                <input type="url" name="link_tombol" value="{{ old('link_tombol', $slider->link_tombol) }}"
                    placeholder="https://..."
                    class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:border-emerald-500 focus:ring-emerald-500/20 transition-all">
            </div>

            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_active" value="1" {{ $slider->is_active ? 'checked' : '' }} id="activeCheck"
                    class="w-5 h-5 rounded text-emerald-600 focus:ring-emerald-500 border-gray-300">
                <label for="activeCheck" class="text-sm font-medium text-gray-700 dark:text-gray-300">Status Aktif</label>
            </div>
    ...
    <script>
        function imagePreview() {
            return {
                imageUrl: "{{ asset('storage/' . $slider->gambar_path) }}",
                previewImage(event) {
