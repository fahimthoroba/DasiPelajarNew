...
@section('title', 'Buat Form Kegiatan')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <p class="text-sm mt-1" style="color: var(--dp-text-secondary)">
            Form publik untuk pendaftaran peserta kegiatan.
        </p>
    </div>
    ...
    <div class="flex items-start gap-4 p-4 rounded-2xl"
        ...
        <div>
            <p class="text-xs font-bold uppercase tracking-widest mb-0.5" style="color: var(--dp-text-secondary)">
                Form ini terkait Program Kerja
            </p>
            <p class="font-bold" style="color: var(--dp-bg-primary)">{{ $proker->nama_proker }}</p>
            <p class="text-xs mt-0.5" style="color: var(--dp-text-secondary)">
                {{ $proker->tgl_pelaksanaan->format('d F Y') }} · {{ $proker->departemen->nama_departemen ?? '-' }}
            </p>
        </div>
    </div>
    @endif
    ...
    {{-- Form --}}
    <form action="{{ route('dashboard.form-kegiatan.store') }}"
          method="POST"
          x-data="prokerFormBuilder([])"
          class="space-y-6">
        @csrf

        @if($proker)
            <input type="hidden" name="program_kerja_id" value="{{ $proker->id }}">
        @endif
        ...
        <div class="p-6 rounded-2xl space-y-4" style="background: var(--dp-bg-surface); border: 1px solid var(--dp-border);">
            ...
            <div>
                ...
                <input type="text" name="nama_kegiatan" required
                       value="{{ $proker ? $proker->nama_proker : old('nama_kegiatan') }}"
                       placeholder="Contoh: Makesta 2026"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="tgl_buka"
                           class="w-full px-4 py-2.5 rounded-xl text-sm"
                           style="background: var(--dp-bg-surface-2); border: 1px solid var(--dp-border); color: var(--dp-text-primary);">
                    <p class="text-[10px] mt-1" style="color: var(--dp-text-secondary)">Kosong = langsung buka</p>
                </div>
                <div>
                    <label class="block text-xs font-bold mb-1.5" style="color: var(--dp-text-secondary)">
                        Ditutup Pada
                    ...
                    <input type="datetime-local" name="tgl_tutup"
                           class="w-full px-4 py-2.5 rounded-xl text-sm"
                           style="background: var(--dp-bg-surface-2); border: 1px solid var(--dp-border); color: var(--dp-text-primary);">
                    <p class="text-[10px] mt-1" style="color: var(--dp-text-secondary)">Kosong = tanpa batas waktu</p>
                </div>
            </div>

            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="is_open" value="1" checked class="w-4 h-4 rounded accent-[#08332c]">
                <div>
                    <span class="text-sm font-bold" style="color: var(--dp-text-primary)">Langsung buka saat dibuat</span>
                    <p class="text-[11px]" style="color: var(--dp-text-secondary)">Peserta bisa langsung mendaftar setelah form dibuat</p>
                </div>
            </label>
        </div>
        ...
        <div class="p-6 rounded-2xl space-y-4" style="background: var(--dp-bg-surface); border: 1px solid var(--dp-border);">
            <div>
                ...
                <p class="text-xs mt-1" style="color: var(--dp-text-secondary)">
                    Tambahkan pertanyaan khusus di luar field standar di atas jika diperlukan.
                </p>
            </div>
                ...
                <template x-for="(field, index) in fields" :key="index">
                    <div class="relative p-4 rounded-xl"
                        ...
                        <button type="button" @click="removeField(index)"
                                class="absolute top-3 right-3 w-7 h-7 rounded-full flex items-center justify-center transition-colors"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pr-10">
                            ...
                            <div class="sm:col-span-2" x-show="field.type === 'select'" x-transition>
                                <label class="block text-[10px] font-bold mb-1" style="color: var(--dp-text-secondary)">
                                    Pilihan (pisahkan dengan koma)
                                </label>
                                <input x-model="field.options" type="text" placeholder="Pilihan A, Pilihan B, Pilihan C"
                                       class="w-full px-3 py-2 rounded-lg text-sm"
                                       style="background: var(--dp-bg-surface); border: 1px solid var(--dp-border); color: var(--dp-text-primary);">
                            </div>
                        </div>
            ...
            <button type="button" @click="addField"
                    class="w-full py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-colors"
        ...
        <div class="flex gap-3">
            @if($proker)
                <a href="{{ route('dashboard.departemen.proker.show', $proker->id) }}"
                   class="flex-1 py-3 rounded-xl text-sm font-bold text-center transition-colors"
