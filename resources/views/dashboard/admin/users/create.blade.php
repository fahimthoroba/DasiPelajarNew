...
@section('title', 'Buat Akun Pengguna Baru')
...
@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-display font-bold text-gray-900 dark:text-white">Buat Akun Pengguna</h1>
                <p class="text-gray-500 text-sm mt-1">Tambahkan akun login baru untuk mengakses sistem.</p>
            </div>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-white/5 p-6">
            <form action="{{ route('dashboard.admin.users.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name') }}"
                        placeholder="Contoh: Admin PAC Ngasem"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="role" id="role" required
                            ...
                            <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>Admin Web Penuh</option>
                            <option value="pc" {{ old('role') == 'pc' ? 'selected' : '' }}>Pengurus Cabang</option>
                            <option value="pac" {{ old('role') == 'pac' ? 'selected' : '' }}>Pimpinan Anak Cabang (PAC)</option>
                            <option value="departemen" {{ old('role') == 'departemen' ? 'selected' : '' }}>Lembaga / Departemen PC</option>
                        </select>
                        @error('role')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
